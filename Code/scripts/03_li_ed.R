...
# 04_li_ed_pos.Rmd joins into `both`/`col_users` with data.table's `x[i, on=]`
# syntax; both_final.rds's tibble-ness (04_col_hs_construct.R's as_tibble())
# carries through distinct(), so without this they're plain tibbles and
# `[.tbl_df` gets dispatched instead of `[.data.table` -- the join then
# errors trying to use the join table as a bare row index.
setDT(both)
setDT(col_users)
