...
#x1 = readLines(file.path(directory,"Data/02",edufilename1))
...
#ed_li1 = fread(file.path(directory,"Data/02",edufilename1),
#              drop = c('world_rank'))
...
#ed_li2 = fread(file.path(directory,"Data/02",edufilename2),
 #             drop = c('world_rank'),
#              fill=TRUE,
#              sep = ",")
...
misfit_counts <- misfits[, .N, by = alt_name] %>%
  filter(N > 50)
write.csv(misfit_counts, file.path(directory,"Data/02/misfits_3.csv"))

# Upload manual correction of some large institutions
# Potential expansion possible
misfits_correction <- read_delim(file.path(directory,"Data/02",misfitscorrection)) %>%
...
write.csv(inst_counts,file.path(directory,"Data/02/inst_counts.csv"))
```
