...
# (birth year), geography (state + Census region), transfer status, and
# graduate-degree attainment, for both columns on a common schema wherever
# the underlying data supports it.
#
# Run after: memo1_column1_construct.R (Column 1's population + degree
# enrichment) and memo1_column1_demo.R (Column 1's demographics) have both
# completed. Reads column2_regression_refresh.rds directly -- no dependency
# on this file existing being persisted by any run_pipeline*.R script (see
# HANDOFF.md's open item on that gap; not this script's job to fix).
#
# Standalone script, not part of run_pipeline.R, per this repo's
# no-source()-between-files convention. All joins below use merge()
# (explicit by.x/by.y, all.x=TRUE), not data.table bracket-chaining, to
# keep column provenance unambiguous in a script composed fresh rather than
# mirrored line-for-line from existing pipeline code.
#
...
# same exception to the no-source() convention memo1_column1_construct.R
# already makes. Used below to attach col_cbsa_code to Column 1, which
# memo1_column1_construct.R's narrower column1_col_match schema never
# carried through (unlike Column 2, which already has col_cbsa_code natively
# via 05_merge.Rmd).
...
# memo1_column1_construct.R's Section 1). Investigate if this comes in low.
