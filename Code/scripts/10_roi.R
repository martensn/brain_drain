...
# #  filter(col_end > 1999) %>%
# #  count(col_unitid,col_end) %>%
# #  rename(year = col_end) %>%
# #  select(-c("V1")) %>%
# #  mutate(col_unitid = as.numeric(col_unitid))
# 
...
# 
# # Create the `in_state` and `recruited` columns
# microdata[, in_state := ifelse(hs_state == col_state, "In-State", "Out-of-State")]
# microdata[, recruited := fifelse(in_state == "Out-of-State" & same_state_col == 1, "Recruited",
#                         fifelse(in_state == "Out-of-State" & same_state_col == 0, "Not_Recruited",
#                         fifelse(in_state == "In-State" & same_state_col == 1, "Retained", "Not_Retained")))]
# microdata[, col_unitid := as.integer(col_unitid)]
# 
...
instate = raw_instate %>%
  ...
  mutate(in_state = case_when(fips == state_of_residence ~ "instate",
                              TRUE ~ "total")) %>%
  select(-c(state_of_residence,type_of_freshman)) %>%
...
sumstat_instate = institutional_characteristics %>%
  ungroup(unitid) %>%
  group_by(inst_group) %>%
  filter(!is.na(shr_instate_ipeds) & !is.na(total_deg_awarded)) %>%
  summarize(shr_instate = weighted.mean(shr_instate_ipeds,w=total_us)*100) %>%
  pivot_longer(!inst_group, names_to = "metric", values_to = "value") %>%
  pivot_wider(names_from = inst_group, values_from = value)
...
sumstat = rbind(sumstat_major,sumstat_acceptance,sumstat_instate,sumstat_mobility,sumstat_finance,sumstat_gradrate)

sumstat$Measure <- c("Agriculture","Business","Education","Engineering",
                   "Humanities","Physical Sciences","Services","Social Sciences",
                   "Healthcare","Other","Acceptance Rate","In-State","Mobility Rate",
                   "Parent Mean Income (2013)","Kid Mean Income (2013)","Parents From Bottom 60%",
                   "Parents From Top 20%","Parents From Top 1%","Kids in Bottom 60%",
                   "Kids in Top 20%","Kids in Top 1%","State Appropriations % of Revenue",
                   "Tuition % of Revenue","Completed in 4 Years","Completed in 6 Years","Completed in 8 Years")

sumstat <- sumstat %>%
  relocate(Measure, .before = "PF") %>%
  select(-c(metric,`Private, For-Profit`))
ss <- xtable(sumstat,
       digits = c(0,0,1,1,1))
print(ss,file=file.path(directory,"Outputs/Tables","sumstat_dec.tex"),include.rownames = FALSE)
...
roi <- merged %>%
  ...
  left_join(recruitment, by = "unitid") %>%
  # Calculate in-state completions and costs per graduate
  mutate(in_state = (shr_recruited + shr_retained) * completion_rate_200pct * cohort_rev,
...
#    "Retained Share",
    ...
    "Graduates per $100k in Tuition Less Non-Completers"))

# Convert to xtable for LaTeX export
#roi_table <- xtable(roi_group, caption = "Summary Statistics", label = "tab:roi-group")
...
# Filter to exclude Non-Completer variables
roi_group_no_nc <- roi_group %>%
  filter(!metric %in% c("State Cost Less Non-Completers", 
                        "Tuition Cost Less Non-Completers",
                        "Graduates per $100k in Appopriations Less Non-Completers",
                        "Graduates per $100k in Tuition Less Non-Completers"))

# Convert to xtable for LaTeX export
roi_table_no_nc <- xtable(roi_group_no_nc, caption = "Summary Statistics Excluding Non-Completers", label = "tab:roi-group-no-nc")

# Print the LaTeX table with labels and caption
print(roi_table_no_nc, file = file.path(directory,"/Outputs/Tables/roi_group_no_nc.tex"),include.rownames = FALSE, hline.after = c(-1, 0, nrow(roi_group_no_nc)), caption.placement = "top")

# Filter to include only Non-Completer variables
roi_group_nc <- roi_group %>%
  filter(metric %in% c("State Cost Less Non-Completers", 
                       "Tuition Cost Less Non-Completers",
                       "Graduates per $100k in Appopriations Less Non-Completers",
                       "Graduates per $100k in Tuition Less Non-Completers"))
...
# Convert to xtable for LaTeX export
roi_table_nc <- xtable(roi_group_nc, caption = "Summary Statistics for Non-Completers", label = "tab:roi-group-nc")

# Print the LaTeX table with labels and caption
print(roi_table_nc, file = file.path(directory,"/Outputs/Tables/roi_group_nc.tex"), include.rownames = FALSE, hline.after = c(-1, 0, nrow(roi_group_nc)), caption.placement = "top")
