...
for(a in avgs) 
{
  ...
  for(u in universes)
  {
    ...
    if(u == "stay") 
    {
      ...
      form <- paste0("hs_growth_",a," ~ hs_shock_",a," +",covariates)
      hs_first_stay <- fixest::feols(as.formula(form),
                                     data = data,
    ...
    if(u == "leave_hs") 
    {
      ...
      hs_first_form <- paste0("hs_growth_",a," ~ hs_shock_",a," + col_shock_",a," +",covariates)
      hs_first <- fixest::feols(as.formula(hs_first_form),
                                      data = data,
                                      cluster = ~hs_cbsa_code)
      col_first_form <- paste0("col_growth_",a," ~ hs_shock_",a," + col_shock_",a," +",covariates)
      col_first <- fixest::feols(as.formula(col_first_form),
                                       data = data,
                                       cluster = ~hs_cbsa_code)
      print(paste("Solved: First-stage Unfitted",u,a))

      # Extract fitted values
      data$hs_shock_fit = fitted(hs_first)
      data$col_shock_fit = fitted(col_first)
        
      # Re-run first-stage controlling for the fitted values of the other shock
      print(paste("Solving: First-stage Fitted",u,a))
      hs_second_form <- paste0("hs_growth_",a," ~ hs_shock_",a," + col_shock_fit +",covariates)
      hs_second <- fixest::feols(as.formula(hs_first_form),
                                 data = data,
                                 cluster = ~hs_cbsa_code)
      col_second_form <- paste0("col_growth_",a," ~ hs_shock_fit + col_shock_",a," +",covariates)
      col_second <- fixest::feols(as.formula(col_first_form),
                                  data = data,
                                  cluster = ~hs_cbsa_code)
  ...
  texreg(
    ...
    custom.gof.rows = list(
        "F-Statistic" = sapply(list(hs_first_stay, hs_first, hs_second, col_first, col_second), 
                               function(model) fixest::fitstat(model, type = "f")[["f"]]$stat)
    ),
    file = paste0(directory, "/Outputs/Tables/first_stage_",a,".tex"),
    append = TRUE
  )
  # Extract F-statistic and R-squared, store in list
  stay_stats[[as.character(a)]] <- list(
    f_stat = fixest::fitstat(hs_first_stay, type = "f")[["f"]]$stat,
    r_squared = r2(hs_first_stay)[["r2"]],
    intercept = coef(hs_first_stay)[["(Intercept)"]],
    intercept_se = se(hs_first_stay)[["(Intercept)"]],
    slope = coef(hs_first_stay)[[paste0("hs_shock_",a)]],
    slope_se = se(hs_first_stay)[[paste0("hs_shock_",a)]]
  )
  leave_hs_stats[[as.character(a)]] <- list(
    f_stat = fixest::fitstat(hs_second, type = "f")[["f"]]$stat,
    r_squared = r2(hs_second)[["r2"]],
    intercept = coef(hs_second)[["(Intercept)"]],
    intercept_se = se(hs_second)[["(Intercept)"]],
    slope = coef(hs_second)[[paste0("hs_shock_",a)]],
    slope_se = se(hs_second)[[paste0("hs_shock_",a)]]
  )
  leave_col_stats[[as.character(a)]] <- list(
    f_stat = fixest::fitstat(col_second, type = "f")[["f"]]$stat,
    r_squared = r2(col_second)[["r2"]],
    intercept = coef(col_second)[["(Intercept)"]],
    intercept_se = se(col_second)[["(Intercept)"]],
    slope = coef(col_second)[[paste0("hs_shock_",a)]],
    slope_se = se(col_second)[[paste0("hs_shock_",a)]]
  )
  ...
  captions[[paste("stay",a,sep="_")]] <- paste0("First-stage F-statistic for high school and college LD shock is ",
                               round(get("stay_stats")[[as.character(a)]]$f_stat)," ($R^2=",
                                    round(get("stay_stats")[[as.character(a)]]$r_squared,3),"$)")
  captions[[paste("leave_hs",a,sep="_")]] <- paste0("First-stage partial F-statistic for high school LD shock is ",
                                    round(get("leave_hs_stats")[[as.character(a)]]$f_stat)," ($R^2=",
                                    round(get("leave_hs_stats")[[as.character(a)]]$r_squared,3),"$) and ",
                                    round(get("leave_col_stats")[[as.character(a)]]$f_stat)," for college shock ($R^2=",
                                    round(get("leave_col_stats")[[as.character(a)]]$r_squared,3),"$).")
