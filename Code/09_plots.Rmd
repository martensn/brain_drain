...
```
###     Occupations
####   Plotting Occupational Composition
```{r occp}

li_soc = read_csv(file.path(directory,"Data/07","soc_li_distribution.csv"),col_select = -`...1`) %>%
  mutate(socp = substr(soc_code_mode,1,7)) %>%
  group_by(socp) %>%
  summarize(shr_li = sum(shr_li))

# Import
soc_ba = read_csv(file.path(directory,"Data/07","soc_ba.csv"),col_select = -`...1`) %>%
...
####   Largest Occupations by Prestige Bin
```{r prestige-bins}
...
####    Cross-Sectional Prestige Distribution by Inst Grouping
...
raw_return <- fread(file.path(directory,"Data/05","return_occp.csv"))

# Obtain weights and institutional groupings from institutional_characteristics 
inst_weights <- read_csv(file.path(directory,"Data/06","institutional_characteristics.csv")) %>%
...
return_by_year <- fread(file.path(directory,"Data/07","return_by_year.csv")) %>%
...
####    return_levels.png + leaver_levels.tex
...
###   Education Imputation Checks

####   Modal State Computation
...
####    Cleaning Grads on the Go
https://www.openicpsr.org/openicpsr/project/170381/version/V4/view?path=/openicpsr/170381/fcr:versions/V4/Data/CollegeMarketShares_state.csv&type=file 
Conzelmann, Johnathan, Hemelt, Steven, Hershbein, Brad, Martin, Shawn, Simon, Andrew, and Stange, Kevin. Grads on the Go: Measuring College-Specific Labor Markets for Graduates [US, 2010-2018]: Data: CollegeMarketShares_state.csv. Ann Arbor, MI: Inter-university Consortium for Political and Social Research [distributor], 2023-12-06. https://doi.org/10.3886/E170381V4-156446
```{r}
institutional_characteristics = read_csv(file.path(directory,"Data/06","institutional_characteristics.csv"))

# raw_deg_award is an RDS file (no .rds extension) written by new/00_alias_generation.R
raw_deg_award = readRDS(file.path(directory,"Data","raw_deg_award"))
...
gotg = fread(file.path(directory,"Data/06/CollegeMarketShares_state.csv"),drop="V1")
...
```
####    Plotting Grads on the Go Benchmark
...
###   Plotting Coefficients by Universe
...
pooled_instate = raw_instate %>%
  ...
  left_join(read_csv(file.path(directory,"Data/06","institutional_characteristics.csv")) %>% dplyr::select(c(unitid,inst_group)), by = "unitid") %>%
...
###   Semi-Elasticity Back of Envelope
...
#raw_lm = fread(file.path(directory,"Data/04","cbp19co.txt"),
...
envelope_long = envelope %>%
  left_join(read_csv(file.path(directory,"Data/06","institutional_characteristics.csv")) %>% 
              group_by(inst_group) %>% 
...
cbsa_shock <- fread(file.path(directory,"Data/04","cbsa_shock.csv"))
...
###   Clean Intercensal Data for Yagan

```{r intercensal, eval=FALSE}
...
###    Joint Occp-Inst-Bin Distribution
...
### Pull Degree Data for Sumstat

```{r deg pull, eval=FALSE}
...
write.csv(total_deg,file.path(directory,"Data/06/total_deg.csv"), row.names = FALSE)
...
write.csv(business_deg,file.path(directory,"Data/06/business_deg.csv"), row.names = FALSE)
...
write.csv(engineering_deg,file.path(directory,"Data/06/engineering_deg.csv"), row.names = FALSE)
...
write.csv(all_deg,file.path(directory,"Data/06/all_deg.csv"),row.names = FALSE)

```


###   Full Labor Force Sumstat
...
cohort_x_grad <-fread(file.path(directory,"Data/06/cohort_x_grad.csv"))
...
total_deg <- read_csv(file.path(directory,"Data/06/total_deg.csv"))
business_deg <- read_csv(file.path(directory,"Data/06/business_deg.csv")) %>%
  rename(business_awards = awards)
engineering_deg <- read_csv(file.path(directory,"Data/06/engineering_deg.csv")) %>%
