...
consolidation (Code/new/00_alias_generation.R through 04_col_hs_construct.R:
...
logic collapses considerably. Known gap, not a priority right now:
`both_final.rds` doesn't track start dates, so `hs_start`/`col_start` are NA
here -- richer age imputation using college start date is a follow-up.
