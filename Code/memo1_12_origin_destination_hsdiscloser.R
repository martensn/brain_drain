# memo1_12_origin_destination_hsdiscloser.R
#
# [CONSOLIDATED 2026-08-24, per Nicholas's request to reduce script count]
# Merges what were two files (memo1_12a_origin_destination_hsdiscloser.R,
# memo1_12b_..._export_box.R) into one -- unlike the Column 1 version of
# this pair (kept separate, see memo1_13_...R's header), this build is
# comparatively cheap (~30 min, not ~3.5 hours), so the original rationale
# for a standalone re-run-just-the-export path (recovering from an export
# bug without repeating an expensive build) has much less value here.
# Section 2 now reuses Section 1's in-memory `out` directly instead of
# re-reading the CSV from disk. No other logic changed from either
# original script.
#
# SECTION 1 (was memo1_12a..., was college_origin_destination_counts.R):
# Standalone descriptive output, NOT part of Memo 1's own write-up. For
# each college (col_unitid x col_opeid), counts of students by ORIGIN
# labor market (home CBSA, hs_cbsa_code -- time-invariant, same every year
# for a given person) and DESTINATION labor market (current CBSA, split
# out by CALENDAR YEAR of observation -- every year, not a single
# snapshot, per Nicholas's explicit follow-up), further split by race and
# sex. NOT split by graduation year (col_end) -- explicitly dropped per
# his first follow-up ("I don't expect a college's migration destinations
# to change all that much over time -- I'd rather look at differences
# across sub-groups"). Calendar year of OBSERVATION and graduation-year
# COHORT are different axes; only the latter was dropped.
#
...
# at all (see memo1_13_origin_destination_fullsample.R).
#
...
#
# SECTION 2 (was memo1_12b..., was college_origin_destination_export_box.R):
# Filters Section 1's output to calendar_year 2012-2023 (Phase B's own
# calibration window, where w2 is actually defined) and writes two
# Box-syncable exports -- a plain version and a college_name-joined,
# col_unitid-sorted version. Sorting before saveRDS is deliberate, not
# cosmetic: grouping identical college_name strings together lets R's
# serializer compress far better -- confirmed against the pre-refresh
# stale Box files, where the named+sorted version (475MB) was smaller on
# disk than the unsorted plain version (897MB) despite carrying one MORE
# column. Both written to Box\Claude-Settings\Plans\BRAIN_DRAIN\data\ for
...
## ===========================================================================
## SECTION 1: build the full college x origin x destination x year x race x
## sex table
## ===========================================================================
log_step("SECTION 1: building college_origin_destination_counts_by_year")
...
## ---- Person-year accumulation: one t-loop, every t where this person has
## a defined destination CBSA that year. w2 looked up per t from the
...
## ===========================================================================
## SECTION 2: Box export -- 2012-2023 window, plain + college-named/sorted
## ===========================================================================
if (dir.exists(box_dir)) {
  log_step("SECTION 2: Box export")
  log_step("Filtering to calendar_year 2012-2023")
  windowed <- out[calendar_year %in% 2012:2023]
...
log_step("memo1_12_origin_destination_hsdiscloser.R done.")
