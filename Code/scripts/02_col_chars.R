...
# bind_rows(), not rbindlist(): get_education_data() returns a tibble, and
# downstream code (deg_award_total, deg_award_super_opeid, etc.) is written
# assuming plain dplyr semantics on that tibble. rbindlist() always returns
# a data.table regardless of its inputs' class, which silently changed
# raw_deg_award's type and broke a later table.express-dispatched join
# further downstream that wasn't written to receive a real data.table here.
raw_deg_award = bind_rows(lapply(2000:2021, function(yr) {
...
institutional_characteristics = raw_institutions %>%
  ...
  mutate(aau = if_else(is.na(aau),0,aau),
         # [FIXED -- 2026-07-29] land_grant==1/2 (not-land-grant/land-grant)
         # was the old live get_education_data() API's encoding. colleges.rds
         # (built from a static IPEDS directory CSV, Code/01a_alias_generation.R)
         # encodes the same concept as a 0/1 flag instead (0=not land-grant,
         # 1=land-grant) -- verified against known land-grant flagships
         # (Penn State/Ohio State/Purdue/Auburn Main Campus all land_grant==1,
         # 85 total, matching the real-world count). Reusing the old 1/2
         # thresholds against the new 0/1 field silently zeroed out the
         # entire RPU category (every land-grant public university
         # misclassified as "PF"), caught when 08_data_generation.Rmd's
         # complier regression errored on a constant dependent variable for
         # inst_group=="RPU" -- zero such rows existed post-fix, until now.
         inst_group = case_when(inst_control == 1 & (land_grant == 0 | aau == 1) ~ "PF",
                                inst_control == 1 & land_grant == 1 & aau == 0 ~ "RPU",
