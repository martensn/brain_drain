...
## [FIXED -- 2026-07-29] soc_li_distribution.csv/soc_ba.csv are both written
## with row.names=FALSE (05_merge.Rmd) -- no stray index column to drop.
## col_select=-`...1` errored ("Column `...1` doesn't exist") once this
## chunk was actually reached on a fresh run for the first time this
## session. Left the other ~9 col_select=-`...1` reads in this file alone --
## their source writes in 08_data_generation.Rmd genuinely lack
## row.names=FALSE, so they really do have the stray column.
li_soc = read_csv(file.path(data_dir,"intermediate/soc_li_distribution.csv")) %>%
  mutate(socp = substr(soc_code_mode,1,7)) %>%
  group_by(socp) %>%
  summarize(shr_li = sum(shr_li))

# Import
soc_ba = read_csv(file.path(data_dir,"intermediate/soc_ba.csv")) %>%
...
## [FIXED -- 2026-07-29] Was c(25,10,5) -- 25 has never been a real bin
## width anywhere upstream. 06_finalize_data.Rmd/08_data_generation.Rmd's
## own num_bins is c(5,10); no returners_*_25.csv has ever existed under
## any specification (confirmed against an old pre-session Data/pre_2000/
## snapshot too), so this predates this session. Corrected to match what
## 08_data_generation.Rmd actually generates.
num_bins = c(10,5)
...
## [FIXED -- 2026-07-29] raw_lm's construction was commented out (presumably
## to skip re-reading this ~117MB file while iterating interactively, relying
## on it already being loaded in-session) but the line right below still
## used it live -- errors "object 'raw_lm' not found" on any fresh run.
## File confirmed present on disk; restoring the read.
...
## [FIXED -- 2026-07-29] institutional_characteristics.csv is written with
## row.names=FALSE (02_col_chars.Rmd) -- no stray index column to drop; see
## the same fix earlier in this file for soc_li_distribution.csv/soc_ba.csv.
institutional_characteristics = read_csv(file.path(data_dir,"intermediate/institutional_characteristics.csv")) %>%
