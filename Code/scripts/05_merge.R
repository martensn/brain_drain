...
# pull (get_education_data(..., filters=list(year=2021))) for col_cbsas,
# joined via a default inner merge() -- any col_unitid not in that one
# snapshot silently dropped the entire row (109,441 of them: Penn State Main
# Campus, University of Phoenix-Arizona, Kennesaw State, etc. -- likely IPEDS
# unitid consolidation/splits across years, not a real absence). colleges.rds
# (built in 00_alias_generation.R) already covers these institutions via a
# comprehensive 2000-2024 pull, and its col_fips is already zero-padded, so
# no county_fips/str_pad re-derivation is needed. resolve_college()
# (Code/college_lookup.R) picks the era-correct row for the ~10% of unitids
# that were renamed/re-OPEID'd over time, using each user's own col_end, and
# preserves every row regardless of match (equivalent to all.x=TRUE) --
# together with the change below, geo lookup failures now NA the CBSA
# columns instead of silently dropping the person. See
# yes-let-s-resolve-this-misty-kahan.md's 2026-07-28 finding.
colleges_lookup <- readRDS(file.path(data_dir, "intermediate/colleges.rds"))
setDT(colleges_lookup)
setDT(raw_microdata)

raw_microdata <- resolve_college(raw_microdata, "col_unitid", "col_end", colleges_lookup,
                                  select_cols = c("col_fips"))
raw_microdata[, col_cnty := col_fips]
raw_microdata[, col_fips := NULL]
rm(colleges_lookup)
...
# [CHANGED 2026-07-27 -- Phase 2 arrow rewiring] col_end's upper bound predates
# work_history_end's extension to 2025 (04_li_ed_pos.Rmd) -- traced via a
# diagnostic waterfall: of ~895K rows lost in this file, 880,251 failed
# col_end < 2023 alone, almost entirely 2023-2025 graduates the new, wider
# position-year coverage newly made visible upstream, only to be dropped right
# back out here. Extended to 2026 to match, per user decision. (The two
# instances here already disagreed with each other pre-existing -- 2022 vs
# 2023 -- both now aligned to the same bound.)
...
write.csv(soc_merge,file.path(data_dir,"intermediate/soc_merge.csv"), row.names=FALSE)
